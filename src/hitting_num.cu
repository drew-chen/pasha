...
void init_hitting_num_gpu(unsigned_int LParam, unsigned_int vertexExpParam, byte* edgeArray, unsigned_int numEdges, float* D, unsigned_int dSize) {
...
__global__ calc_num_starting_paths_one_iter_gpu(int j) {
   ...
   */
   // want tid range [0, vertexExp)
    ...
    for (unsigned_int j = 1; j <= L; j++) {
        calc_num_starting_paths_gpu<<<grid_size, NUM_THREADS>>>(j); 
    }
}